...
TabLegacy:Button({
    ...
    Justify = "Center",
    Callback = function()
        local scannedCount = forceScanCactus()
        if scannedCount > 0 then
            WindUI:Notify({ Title = "Cactus", Content = "Done Hitbox! " .. tostring(scannedCount) .. " cây xương rồng!", Type = "Success" })
        else
            WindUI:Notify({ Title = "Cactus", Content = "Not found!", Type = "Warning" })
        end
    end
})
...
TabVisuals:Toggle({ 
    Title = "Enable Headless Head Mode", 
    Value = false, 
...
TabVisuals:Toggle({ 
    Title = "Enable Korblox Right Leg Mode", 
    Value = false, 
    Callback = function(state) 
...
TabVisuals:Section({ Title = "Aura Color Properties (Đổi Màu Vật Phẩm)" })
...
TabVisuals:Section({ Title = "12 Owned Emotes (Tên Hàng Gốc Bạn Có)" })
for i = 1, 12 do
    uiInputBoxesOwned[i] = TabVisuals:Input({
        Title = string.format("Slot %02d [Owned Emote]", i), Placeholder = "Gõ tên chính xác Emote gốc...",
...
TabVisuals:Section({ Title = "12 Target Emotes (Tên Hàng Muốn Đổi Sang)" })
for i = 1, 12 do
    uiInputBoxesTarget[i] = TabVisuals:Input({
        Title = string.format("Slot %02d [Swap Target]", i), Placeholder = "Gõ tên chính xác Emote muốn tráo...",
...
TabVisuals:Button({
    ...
    Callback = function()
        ...
        WindUI:Notify({ Title = "Emote System", Content = "Done " .. tostring(activeCount) .. " Slots!", Type = "Success" })
    end
})
...
TabVisuals:Button({
    ...
    Callback = function()
        for i = 1, 12 do
            ...
            selectEmotes[i] = ""
            emoteEnabled[i] = false
            pcall(function() uiInputBoxesOwned[i]:SetValue("") end)
            pcall(function() uiInputBoxesTarget[i]:SetValue("") end)
        end
        WindUI:Notify({ Title = "Emote System", Content = "Reset Done", Type = "Warning" })
    end
})
...
-- Sửa lỗi: Thêm Cooldown kiểm tra giây tránh spam nhảy liên tục gây treo màn hình
    ...
    if isActive then
        ...
        if hum and hrp and hum.Health > 0 then
            ...
            if isCharacterGrounded(char, hrp, hum) and (now - LastJump) > 0.15 then
                LastJump = now -- Chặn ngay lập tức trước khi chạy sự kiện tiếp theo
                pcall(function() 
