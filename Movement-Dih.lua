...
CustomHitboxFolder.Name = "MovementDih_CustomHitboxes"
CustomHitboxFolder.Parent = workspace
local mapID = ""
local spawnedMap = nil
local globalColorValue = Color3.fromRGB(255, 255, 255)
local globalColorOriginals = {}
local globalColorApplied = false
local hideOthersEnabled = false
local hideSelfEnabled   = false
local hideOthersConn    = nil
local removeInvisWallsEnabled = false
local invisWallOriginals = {}
...
local function feSetupEmoteSpeedChange(apply)
    if feEmoteSpeedConn then feEmoteSpeedConn:Disconnect(); feEmoteSpeedConn = nil end
        ...
        if mt and mt.__namecall then
            ...
            mt.__namecall = function(self, ...)
                ...
                if method == "FireServer" and self == emoteRemoteFE then
                    ...
                    if num:match("^[1-6]$") and feBlockRemote then
            ...
            if isGrounded then
                ...
                elseif accelerationMethod == "Ground Acceleration" or accelerationMethod == "Acceleration" then
                    ...
                    if AutoAccelerationEnabled then
                        ...
                        if currentSpeed > MaxSpeed then finalFriction = MaxAcceleration
                        else finalFriction = MinAcceleration end
                    end
...
function applyBhopFriction(forceReset)
    if not forceReset and #frictionTables == 0 and getgenv().bhopHoldSystemEnabled then
        for _, obj in pairs(getgc(true)) do
...
local function onBouncePostSimulation()
    ...
    if hum and hrp and hum.Health > 0 then
        ...
        if isGrounded and not bounceDebounce then
            ...
            task.delay(0.05, function() bounceDebounce = false end)
        end
    end
end
...
local function saveOriginalColors()
    for _, obj in ipairs(workspace:GetDescendants()) do
        if obj:IsA("BasePart") and not globalColorOriginals[obj] and not isPlayerPart(obj) then
            globalColorOriginals[obj] = { Color = obj.Color, Material = obj.Material }
        end
    end
end
local function applyGlobalColor(color)
    saveOriginalColors()
    for _, obj in ipairs(workspace:GetDescendants()) do
...
local function resetGlobalColor()
    for obj, data in pairs(globalColorOriginals) do
        pcall(function()
...
local function refreshHideOthers()
    for _, player in ipairs(game:GetService("Players"):GetPlayers()) do
        if player ~= LocalPlayer then
            setCharacterVisibility(player.Character, not hideOthersEnabled)
        end
    end
end
local function saveAndRemoveInvisWalls()
    for _, obj in ipairs(workspace:GetDescendants()) do
        if obj:IsA("BasePart") then
            if invisWallOriginals[obj] == nil then
                invisWallOriginals[obj] = obj.CanCollide
            end
            if obj.Transparency > 0.5 and obj.CanCollide == true then
...
local function restoreInvisWalls()
    for obj, originalCollide in pairs(invisWallOriginals) do
        pcall(function()
            if obj and obj.Parent then obj.CanCollide = originalCollide end
        end)
    end
    invisWallOriginals = {}
end
            ...
            if Workspace:FindFirstChild("Game") and Workspace.Game:FindFirstChild("Players") then
                ...
                if playerFolder then
                    ...
                    if tagAttr then
                        ...
                        if tagNum and tagNum >= 0 and tagNum <= 255 then currentTag = tagAttr; break end
                    end
                end
            end
            task.wait(0.5)
        ...
        if methodName == "FireServer" and self == EmoteRemote and type(args[1]) == "string" then
            for i = 1, 12 do
                if emoteEnabled[i] and currentEmotes[i] ~= "" and args[1]:gsub("%s+", ""):lower() == currentEmotes[i]:lower() then
                    ...
                    task.spawn(function()
                        task.wait(0.1); blockOriginalEmote = false
                        if pendingSlot == i then pendingSlot = nil; fireSelectEmote(i) end
                    end)
...
MoveGroup1:AddInput("PlayerJumpHeightPower", { Default = "3.5", Text = "Player Jump Height Power", Placeholder = "Default: 3.5", Numeric = true, Callback = function(value) if tonumber(value) then currentSettings.JumpPowerValue = tonumber(value) end end })
MoveGroup1:AddInput("PlayerJumpCapMultiplier", { Default = "1", Text = "Player Jump Cap Multiplier", Placeholder = "Default: 1", Numeric = true, Callback = function(value) local val = tonumber(value) if val then currentSettings.JumpCap = val; applyMovementSettings() end end })
MoveGroup1:AddInput("AirStrafeAcceleration", { Default = "187", Text = "Air Strafe Acceleration", Placeholder = "Default: 187", Numeric = true, Callback = function(value) local val = tonumber(value) if val then currentSettings.AirStrafeAcceleration = val; applyMovementSettings() end end })
MoveGroup2 = Tabs.Movement:AddLeftGroupbox("Bhop")
MoveGroup2:AddToggle("EnableBhopSystem", { Text = "Enable Bhop System (Hold Space)", Default = false, Callback = function(state) getgenv().bhopHoldSystemEnabled = state if not state then getgenv().bhopHoldActive = false end applyBhopFriction(not state) end })
...
MoveGroup8 = Tabs.Movement:AddRightGroupbox("Edge Boost")
MoveGroup8:AddToggle("EnableEdgeBoost", { Text = "Enable Edge Boost", Default = false, Callback = function(state)
...
MoveGroup4:AddSlider("BounceForceVelocity", { Text = "Bounce Force Velocity", Min = 10, Max = 150, Default = 50, Rounding = 0, Callback = function(value) bouncePowerValue = value end })
MoveGroup5 = Tabs.Movement:AddRightGroupbox("Turnbind Engineering")
MoveGroup5:AddToggle("EnableTurnbindModifier", { Text = "Enable Turnbind Modifier", Default = false, Callback = function(state) turnbindEnabled = state end })
MoveGroup5:AddInput("LeftTurnKeyBinding", { Default = "A", Text = "Left Turn Key Binding", Placeholder = "A", Callback = function(v) turnbindLeftKey = v:upper() end })
MoveGroup5:AddInput("RightTurnKeyBinding", { Default = "D", Text = "Right Turn Key Binding", Placeholder = "D", Callback = function(v) turnbindRightKey = v:upper() end })
LegacyGroup1 = Tabs.Legacy:AddLeftGroupbox("Legacy Player Modification")
LegacyGroup1:AddInput("RealSpeedMultiplier", { Default = "1500", Text = "Real Speed Multiplier", Placeholder = "Default: 1500", Numeric = true, Callback = function(value) local n = tonumber(value) if n then RealSpeed = n end end })
LegacyGroup1:AddInput("LegacyJumpHeight", { Default = "3", Text = "Legacy Jump Height", Placeholder = "Default: 3", Numeric = true, Callback = function(value) local n = tonumber(value) if n then JumpHeight = n local char = LocalPlayer.Character local hum = char and char:FindFirstChildOfClass("Humanoid") if hum then hum.JumpHeight = n end end end })
LegacyGroup1:AddInput("AirStrafeAccelerationLegacy", { Default = "187", Text = "Air Strafe Acceleration", Placeholder = "Default: 187", Numeric = true, Callback = function(value) local n = tonumber(value) if n then AirStrafeAcceleration = n end end })
LegacyGroup2 = Tabs.Legacy:AddLeftGroupbox("Legacy Bhop")
LegacyGroup2:AddToggle("EnableBhopAutoJump", { Text = "Enable Bhop (Auto Jump)", Default = false, Callback = function(state) autoJumpEnabled = state checkBhopState() reapplyModifications() end })
LegacyGroup2:AddToggle("BhopKeyHoldingSpace", { Text = "Bhop Key Holding (Space / Mobile Jump)", Default = false, Callback = function(state) bhopHoldFeature = state if not state then bhopHoldActive = false checkBhopState() reapplyModifications() end end })
LegacyGroup2:AddDropdown("BhopModeDropdown", { Values = {"No Acceleration", "Ground Acceleration", "Acceleration"}, Default = "Acceleration", Text = "Bhop Mode", Callback = function(value) accelerationMethod = value reapplyModifications() end })
LegacyGroup2:AddInput("BhopAccelerationInput", { Default = "-0.5", Text = "Bhop Acceleration (Negative Value Only)", Placeholder = "-0.5", Callback = function(value) local n = tonumber(value) if n then groundFriction = n reapplyModifications() end end })
LegacyGroup3 = Tabs.Legacy:AddRightGroupbox("Legacy Auto Acceleration")
LegacyGroup3:AddToggle("EnableAutoAccelerationLegit", { Text = "Enable Auto Acceleration (Legit)", Default = false, Callback = function(state) AutoAccelerationEnabled = state reapplyModifications() end })
LegacyGroup4 = Tabs.Legacy:AddRightGroupbox("Legacy Exploits & Utilities")
...
LegacyGroup6 = Tabs.Legacy:AddLeftGroupbox("Remove Invisible Walls")
LegacyGroup6:AddToggle("EnableRemoveInvisWalls", { Text = "Remove Invisible Walls", Default = false, Callback = function(state)
    removeInvisWallsEnabled = state
    if state then
        saveAndRemoveInvisWalls()
    else
        restoreInvisWalls()
    end
end })
LegacyGroup6:AddButton({ Text = "Refresh Walls", DoubleClick = false, Func = function()
    if removeInvisWallsEnabled then
        restoreInvisWalls()
        saveAndRemoveInvisWalls()
    else
        restoreInvisWalls()
...
LegacyGroup5 = Tabs.Legacy:AddRightGroupbox("Fast Turn")
LegacyGroup5:AddButton({ Text = "Fast Turn", Func = function() runFastTurn() end })
...
LegacyGroupFE = Tabs.Legacy:AddLeftGroupbox("FE Change Emote")
for i = 1, 6 do
    local currentVal = LocalPlayer:GetAttribute("Emote" .. i) or ""
    feEmoteInputs[i] = LegacyGroupFE:AddInput("FEEmoteSlot" .. i, { Default = currentVal, Text = "Emote Slot " .. i, Placeholder = "e.g. Wave, Dab...", Callback = function(value) fePendingEmotes[i] = value:gsub("%s+", "") end })
end
...
LegacyGroupFE:AddToggle("EmoteRunner", { Text = "Emote Runner", Default = false, Callback = function(state)
...
HitboxGroup1 = Tabs.Hitbox:AddLeftGroupbox("Mouse Target Hitbox Settings")
HitboxGroup1:AddToggle("EnableCustomHitboxCreation", { Text = "Enable Custom Hitbox Creation", Default = false, Callback = function(state) customHitboxEnabled = state end })
HitboxGroup1:AddButton({ Text = "Clear All Created Hitboxes", DoubleClick = false, Func = function() CustomHitboxFolder:ClearAllChildren() end })
HitboxGroup2 = Tabs.Hitbox:AddLeftGroupbox("Hitbox Dimensions")
...
HitboxGroup3 = Tabs.Hitbox:AddRightGroupbox("Hitbox Styling")
HitboxGroup3:AddLabel("Select Hitbox Box Color"):AddColorPicker("HitboxColorPicker", { Default = Color3.fromRGB(255, 0, 0), Title = "Select Hitbox Box Color", Callback = function(color) customHitboxColor = color end })
...
HitboxGroup5 = Tabs.Hitbox:AddLeftGroupbox("Hide Players")
HitboxGroup5:AddToggle("HideOtherPlayers", { Text = "Hide Other Players", Default = false, Callback = function(state)
    hideOthersEnabled = state
    refreshHideOthers()
    if state then
        if hideOthersConn then hideOthersConn:Disconnect() end
        hideOthersConn = game:GetService("Players").PlayerAdded:Connect(function(player)
            if hideOthersEnabled then
                player.CharacterAdded:Connect(function(char)
                    task.wait(0.5)
                    if hideOthersEnabled then setCharacterVisibility(char, false) end
                end)
            end
        end)
        for _, player in ipairs(game:GetService("Players"):GetPlayers()) do
        ...
        if hideOthersConn then hideOthersConn:Disconnect(); hideOthersConn = nil end
    end
end })
HitboxGroup5:AddToggle("HideSelfPlayer", { Text = "Hide Myself", Default = false, Callback = function(state)
    hideSelfEnabled = state
    setCharacterVisibility(LocalPlayer.Character, not state)
    SafeConnect(LocalPlayer.CharacterAdded, function(char)
        task.wait(0.5)
        if hideSelfEnabled then setCharacterVisibility(char, false) end
    end)
end })
HitboxGroup4 = Tabs.Hitbox:AddRightGroupbox("Map System")
HitboxGroup4:AddInput("MapIDInput", { Default = "", Text = "Map Asset ID", Placeholder = "e.g. 12345678", Numeric = true, Callback = function(value) mapID = value:gsub("%D", "") end })
HitboxGroup4:AddButton({ Text = "Create Map", DoubleClick = false, Func = function()
    ...
    pcall(function()
        ...
        if obj then
            spawnedMap = obj
            spawnedMap.Name = "MovementDih_CustomMap"
            spawnedMap.Parent = workspace
            if spawnedMap:IsA("Model") then spawnedMap:MoveTo(Vector3.new(0, 2500, 0)) end
        end
    end)
end })
...
HitboxGroup4:AddButton({ Text = "Delete Map", DoubleClick = false, Func = function()
    if spawnedMap then
        spawnedMap:Destroy(); spawnedMap = nil
    else
        local backup = workspace:FindFirstChild("MovementDih_CustomMap")
        if backup then backup:Destroy() end
    end
...
VisualGroup2:AddButton({ Text = "Reset Cosmetic & Restore Original", Func = function()
    resetCosmetics()
...
VisualGroupCC:AddSlider("CustomAuraScaleSlider", { Text = "Aura Scale", Min = 0.1, Max = 10, Default = 1, Rounding = 1, Callback = function(value) customAuraScale = value end })
VisualGroupCC:AddSlider("CustomAuraTransparencySlider", { Text = "Aura Transparency", Min = 0, Max = 1, Default = 0, Rounding = 2, Callback = function(value) customAuraTransparency = value end })
...
VisualGroup5 = Tabs.Visuals:AddRightGroupbox("Emote Remap Action Buttons")
...
LightGroup1:AddSlider("LightBrightness", {
    Text = "Brightness", Min = 0, Max = 10,
    Default = math.floor(Lighting.Brightness * 10 + 0.5) / 10,
    Rounding = 1,
    Callback = function(value) Lighting.Brightness = value end
})
LightGroup1:AddSlider("LightSaturation", {
    Text = "Saturation", Min = -1, Max = 1, Default = 0, Rounding = 2,
    Callback = function(value) colorCorrection.Saturation = value end
})
LightGroup1:AddSlider("LightContrast", {
    Text = "Contrast", Min = -1, Max = 1, Default = 0, Rounding = 2,
    Callback = function(value) colorCorrection.Contrast = value end
})
LightGroup1:AddSlider("LightExposure", {
    Text = "Exposure", Min = -1, Max = 1, Default = 0, Rounding = 2,
    Callback = function(value) colorCorrection.Brightness = value end
})
LightGroup1:AddSlider("LightClockTime", {
    Text = "Time of Day", Min = 0, Max = 24,
    Default = Lighting.ClockTime, Rounding = 1,
    Callback = function(value) Lighting.ClockTime = value end
})
LightGroup1:AddButton({
    Text = "Reset Lighting to Original",
    Func = function() resetLighting() end
})
local LightGroupSun = Tabs.Lighting:AddLeftGroupbox("Sun Rays")
LightGroupSun:AddToggle("EnableSunRays", {
    Text = "Enable Sun Rays", Default = false,
    Callback = function(state) sunRaysEffect.Enabled = state end
})
LightGroupSun:AddSlider("SunRaysIntensity", {
    Text = "Intensity", Min = 0, Max = 1, Default = 0.5, Rounding = 2,
    Callback = function(value) sunRaysEffect.Intensity = value end
})
LightGroupSun:AddSlider("SunRaysSpread", {
    Text = "Spread", Min = 0, Max = 1, Default = 0.5, Rounding = 2,
    Callback = function(value) sunRaysEffect.Spread = value end
})
local LightGroupAtmo = Tabs.Lighting:AddLeftGroupbox("Fog & Sky Temperature")
LightGroupAtmo:AddSlider("AtmoFogDensity", {
    Text = "Fog Density", Min = 0, Max = 1,
    Default = atmosphere and atmosphere.Density or 0, Rounding = 2,
    Callback = function(value) getOrCreateAtmosphere().Density = value end
})
LightGroupAtmo:AddLabel("Fog Color"):AddColorPicker("AtmoFogColor", {
    Default = atmosphere and atmosphere.Color or Color3.fromRGB(199, 199, 199),
    Title = "Fog Color",
    Callback = function(color) getOrCreateAtmosphere().Color = color end
})
LightGroupAtmo:AddSlider("AtmoHaze", {
    Text = "Sky Temperature (Haze)", Min = 0, Max = 10,
    Default = atmosphere and atmosphere.Haze or 0, Rounding = 1,
    Callback = function(value) getOrCreateAtmosphere().Haze = value end
})
LightGroupAtmo:AddLabel("Sky Tint (Warm = orange/red, Cool = blue)"):AddColorPicker("AtmoDecay", {
    Default = atmosphere and atmosphere.Decay or Color3.fromRGB(255, 100, 30),
    Title = "Sky Tint Color",
    Callback = function(color) getOrCreateAtmosphere().Decay = color end
})
local LightGroup2 = Tabs.Lighting:AddRightGroupbox("Sky Box (Model ID)")
LightGroup2:AddInput("SkyboxModelIDInput", {
    Default = "", Text = "Sky Object ID", Placeholder = "Model ID from Toolbox",
    Numeric = true,
    Callback = function(value) customSkyModelID = value:gsub("%D", "") end
})
LightGroup2:AddButton({
    Text = "Apply Sky",
    Func = function() applyCustomSkyModel() end
})
LightGroup2:AddButton({
    Text = "Reset Sky",
...
LightGroup3:AddInput("SkyImageIDInput", {
    Default = "", Text = "Image Asset ID", Placeholder = "e.g. 123280270736635",
    Numeric = true,
    Callback = function(value) customImageSkyID = value:gsub("%D", "") end
})
LightGroup3:AddButton({
    Text = "Apply Image Sky",
    Func = function() applyImageSky() end
})
LightGroup3:AddButton({
    Text = "Reset Sky",
    Func = function() resetSky() end
...
SettingsGroup:AddButton({ Text = "Terminate Script & Clear Cache", Func = function()
    ...
    if edgeBoostConn       then edgeBoostConn:Disconnect() end
    if hideOthersConn      then hideOthersConn:Disconnect() end
    if removeInvisWallsEnabled then restoreInvisWalls() end
    for _, player in ipairs(game:GetService("Players"):GetPlayers()) do
...
do SafeConnect(RunService.Heartbeat, function()
    ...
    if downDashEnabled and LocalPlayer.Character then
        ...
        if hum and hrp and camera then
            ...
            if wasDown and not isDownedState then
                downDashPreActivated = false; downDashActive = false; downDashCurrentSpeed = 0
            end
            if not wasDown and isDownedState then
                if not downDashHasUsed then downDashHasUsed = true end
            end
            ...
            if canDash then
                ...
                if moveDir.Magnitude >= 0.01 then
                    ...
                    hrp.AssemblyLinearVelocity = Vector3.new(moveDir.X * downDashCurrentSpeed, vy, moveDir.Z * downDashCurrentSpeed)
                end
            else
...
SafeConnect(UserInputService.InputBegan, function(input, gameProcessed)
    ...
    if input.KeyCode == Enum.KeyCode.Space then
        if getgenv().bhopHoldSystemEnabled then getgenv().bhopHoldActive = true; applyBhopFriction() end
        if bhopHoldFeature then bhopHoldActive = true; checkBhopState() end
    end
    if downDashEnabled and (input.KeyCode == Enum.KeyCode.LeftControl or input.KeyCode == Enum.KeyCode.C) then
        downDashActive = true
        if not isDownedState then downDashPreActivated = true end
    end
...
SafeConnect(UserInputService.InputEnded, function(input)
    if input.KeyCode == Enum.KeyCode.Space then
        getgenv().bhopHoldActive = false; applyBhopFriction(true)
    ...
    if input.KeyCode == Enum.KeyCode.LeftControl or input.KeyCode == Enum.KeyCode.C then
        ...
        if not isDownedState then downDashPreActivated = false end
        downDashCurrentSpeed = 0
    end
    if turnbindEnabled then
        local leftEnum = parseKeyCode(turnbindLeftKey)
...
local function setupCharacter(character)
    ...
    downDashCurrentSpeed = 0; downDashActive = false; downDashPreActivated = false
    isDownedState = false; downDashHasUsed = false
